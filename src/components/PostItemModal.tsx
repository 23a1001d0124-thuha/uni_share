...
export default function PostItemModal({
  ...
  const [newProdCategory, setNewProdCategory] = useState(CATEGORIES[0] || "Sách & Giáo trình");
  const [newProdCondition, setNewProdCondition] = useState(CONDITIONS[2] || "Còn mới 90% (Xài kỹ, đẹp)");
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          console.warn("Fallback on compression failure in PostItemModal:", err);
  ...
  const handleSmartLensAnalysis = async () => {
    ...
    try {
      ...
      console.error("Smart Lens Error inside PostItemModal:", e);
    } finally {
      setIsScanning(false);
  ...
  const handlePostSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let uploadedImageUrl = newProdImage;
  ...
  return (
    <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-0 md:p-4 animate-fadeIn" id="global-post-modal">
      <form 
                ...
                {!lensResult && (
                  <div className="space-y-3 text-center">
                    ...
                    <p className="text-[11px] text-stone-400 max-w-[280px] mx-auto leading-relaxed">
                      Mô hình Gemini sẽ tự động nhận dạng, đề xuất giá thanh lý và điền danh mục tự động.
                    </p>
                  </div>
                )}
            ...
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4">
              {!pricingAnalysis ? (
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-stone-500 mt-2">Dựa trên tên vật phẩm và tình trạng, Gemini sẽ gợi ý khoảng định giá thanh lý tốt nhất.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      <span className="text-[10px] text-stone-500 block">Biên độ giá hợp lý</span>
                      <span className="font-bold text-stone-800 block mt-0.5">{(pricingAnalysis.suggestedLowerLimit || 0).toLocaleString("vi-VN")} - {(pricingAnalysis.suggestedUpperLimit || 0).toLocaleString("vi-VN")} đ</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-stone-500 block">Mức giá xuất sắc nhất</span>
                      <span className="font-bold text-rose-600 block mt-0.5 text-sm">{(pricingAnalysis.recommendedPrice || 0).toLocaleString("vi-VN")} đ</span>
                    </div>
                    {pricingAnalysis.schoolSuitabilityScore && (
                      <div className="col-span-2">
                        <span className="text-[10px] text-stone-500 block">Điểm Phù Hợp Môi Trường Học Đường (School Suitability Score)</span>
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex-1 h-2 bg-stone-200 rounded-full overflow-hidden">
                            <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${pricingAnalysis.schoolSuitabilityScore}%` }} />
                          </div>
                          <span className="font-bold text-emerald-600 text-xs">{pricingAnalysis.schoolSuitabilityScore}/100</span>
                        </div>
                      </div>
                    )}
                    {pricingAnalysis.suggestedTags && pricingAnalysis.suggestedTags.length > 0 && (
                      <div className="col-span-2">
                        <span className="text-[10px] text-stone-500 block">Gợi ý Thẻ Nổi Bật (Tags)</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {pricingAnalysis.suggestedTags.map((t: string) => (
                            <span key={t} className="bg-white border border-stone-200 text-stone-600 text-[10px] px-2 py-0.5 rounded">#{t}</span>
                          ))}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="bg-white p-2.5 rounded-xl border border-blue-50 text-[11px] text-stone-600 leading-relaxed italic relative">
                    ...
                    <span className="pl-2 block">{pricingAnalysis.reasoning}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setNewProdPrice(String(pricingAnalysis.recommendedPrice))}
            ...
            {!isStudentVerified && (
              <div className="bg-amber-50 border border-amber-205 text-amber-800 p-3 rounded-xl flex gap-2.5 text-xs text-left">
                ...
                <div>
                  ...
                  Tin đăng sẽ thiếu nhãn "SV Verified". Hãy tiến hành <button type="button" onClick={() => { onClose(); onTriggerVerification(); }} className="underline font-bold text-rose-600 cursor-pointer">Xác thực bằng Thẻ sinh viên</button> để nâng cao uy tín.
                </div>
              </div>
            )}
          </div>
